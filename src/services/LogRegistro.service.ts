...
import type { LogEditable, LogLectura } from "../models/logRegistro.ts";
...
export class LogRegistroService {
  ...
  constructor(
    ...
    listName = 'Visitantes - LogVisitas'     
  ) {
    this.graph = graph;
  ...
  // ---------- mapping ----------
  private toModel(item: any): LogLectura {
    const f = item?.fields ?? {};
    const mapped: LogLectura = {
        Id: String(item?.id ?? ''),
        Title: f.Title,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<LogEditable, 'Id'>) {
  ...
  async update(id: string, changed: Partial<Omit<LogEditable, 'Id'>>) {
    await this.ensureIds()
